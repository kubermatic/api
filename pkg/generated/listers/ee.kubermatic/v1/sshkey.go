...
type SSHKeyLister interface {
	...
	List(selector labels.Selector) (ret []*v1.SSHKey, err error)
...
// List lists all SSHKeys in the indexer.
func (s *sSHKeyLister) List(selector labels.Selector) (ret []*v1.SSHKey, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.SSHKey))
	})
	return ret, err
}
...
type SSHKeyNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.SSHKey, error)
...
// List lists all SSHKeys in the indexer for a given namespace.
func (s sSHKeyNamespaceLister) List(selector labels.Selector) (ret []*v1.SSHKey, err error) {
...
// Get retrieves the SSHKey from the indexer for a given namespace and name.
func (s sSHKeyNamespaceLister) Get(name string) (*v1.SSHKey, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("sshkey"), name)
	}
	return obj.(*v1.SSHKey), nil
}
